...
public class SphereSector extends Model implements MeshMaker
{
   ...
   */
                       ...
                       final int n, final int k)
   {
      ...
      // Create all the square strips between phi1 and phi2.
      for (int i = 0; i < n - 1; ++i)
      {
         for (int j = 0; j < k - 1; ++j)
         {
            if ((i == 0) && (phi1 == 0)) // prevent degenerate squares
            {  //                     v[i][j]   v[i+1][j]    v[i+1][j+1]
               addPrimitive(new Face((i*k)+j, ((i+1)*k)+j, ((i+1)*k)+j+1));
            }
            else if ((i == n-2) && (phi2 == Math.PI))
