...
public class Sphere extends Model implements MeshMaker
{
   ...
   */
   public Sphere(final double r, final int n, final int k)
   {
      ...
      for (int j = 0; j < k - 1; ++j)
      {  //                                    v[0][j]  v[0][j+1]
         addPrimitive(new Face(northPoleIndex,   j,       j+1));
      }
      //                                    v[0][k-1]  v[0][0]
      addPrimitive(new Face(northPoleIndex,    k-1,       0));

      // Create the triangle fan at the bottom.
      for (int j = 0; j < k - 1; ++j)
      {  //                                     v[n-1][j+1]    v[n-1][j]
         addPrimitive(new Face(southPoleIndex, ((n-1)*k)+j+1, ((n-1)*k)+j));
      }
      //                                      v[n-1][0]    v[n-1][k-1]
      addPrimitive(new Face(southPoleIndex, ((n-1)*k)+0, ((n-1)*k)+k-1));
   }
