...
public class TriangularPyramid extends Model implements MeshMaker
{
   ...
   */
   public TriangularPyramid(final double r, final double h)
   {
      ...
      // Create 4 triangles.
      addPrimitive(new Face(0, 1, 2),   // bottom face
                   new Face(0, 3, 1),   // face 1
                   new Face(1, 3, 2),   // face 2
                   new Face(0, 2, 3));  // face 3
   }
   ...
   */
                            ...
                            final int n, final int k)
   {
      ...
      {  //                                v[n-1][j+1]       v[n-1][j]
         addPrimitive(new Face(apexIndex, (n-1)*(3*k)+j+1, (n-1)*(3*k)+j));
      }
      //                                 v[n-1][0]     v[n-1][3k-1]
      addPrimitive(new Face(apexIndex, (n-1)*(3*k)+0, (n-1)*(3*k)+3*k-1));
      ...
      for (int i = 0; i < n - 1; ++i) // choose a height of latitude
      {
         ...
         {  //                     v[i][j]    v[i+1][j]       v[i+1][j+1]     v[i][j+1]
            addPrimitive(new Face(i*(3*k)+j, (i+1)*(3*k)+j, (i+1)*(3*k)+j+1, i*(3*k)+j+1));
         }
         //                      v[i][3k-1]      v[i+1][3k-1]       v[i+1][0]     v[i][0]
         addPrimitive(new Face(i*(3*k)+3*k-1, (i+1)*(3*k)+3*k-1, (i+1)*(3*k)+0, i*(3*k)+0));
      }

      // Create the triangle fan in the base.
      for (int j = 0; j < 3*k - 1; ++j)
      {  //                                 v[0][j]  v[0][j+1]
         addPrimitive(new Face(centerIndex,    j,       j+1));
      }
      //                                 v[0][3k-1]  v[0][0]
      addPrimitive(new Face(centerIndex,    3*k-1,      0));
   }
