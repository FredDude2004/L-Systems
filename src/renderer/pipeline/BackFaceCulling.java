...
/**
   ...
   {@link List} of {@link Primitive}s any {@link Face} that
   is "back facing".
<p>
   The culling of {@link Face} primitives needs to be done before the
   primitive assembly stage. The culling of {@link Triangle} primitives
   needs to be done after the primitive assembly stage.
<p>
...
public class BackFaceCulling
{
   ...
   */
   public static Model cull(final Model model, final Camera camera)
   {
      ...
      for (final Primitive p : model.primitiveList)
      {
         ...
         {
            // Keep the non-Face primitives and the Face
            // primitives with the desired orientation.
            newPrimitiveList.add(p);
         }
         else // back facing Face
         {
            if ( ! model.doBackFaceCulling )
            {
               if ( ! model.facesHaveTwoSides )
               {
                  // Keep a back facing Face and
                  // use its front facing colors.
                  newPrimitiveList.add(p);
               }
               else // faces have two sides
               {
                  // Keep a back facing Face but
                  // use its back facing colors.
                  final Face p2 = new Face(p.vIndexList,
                                           ((Face)p).cIndexList2);
   ...
   /**
      Determine if a {@link Primitive} is a {@link Face} and if that Face
      is backward facing. Whether a {@link Face} is back facing or not
      ...
      @param camera  the {@link Scene}'s {@link Camera} with the view volume data
      @return true if {@code p} is a back facing {@link Face}
   */
   public static boolean isBackFace(final Primitive p,
                                    final Model model,
                                    final Camera camera)
   {
      return  (p instanceof Face)
   ...
   /**
      Determine if a {@link Face} is counterclockwise.
      <p>
   ...
   */
                                ...
                                final Camera camera)
   {
      ...
      {
         System.err.println( "4. Culling_1: " + p.toString() );
