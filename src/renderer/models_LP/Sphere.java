...
public class Sphere extends Model implements MeshMaker
{
   ...
   */
   public Sphere(final double r, final int n, final int k)
   {
      ...
      // Create line fans at the north and south poles.
      final LineFan lineFanN = new LineFan(),
                    lineFanS = new LineFan();
      lineFanN.addIndex(northPoleIndex);
      lineFanS.addIndex(southPoleIndex);
      for (int j = 0; j < k; ++j)
      {
         lineFanN.addIndex(     (0 * k) + j ); // v[  0][j]
         lineFanS.addIndex( ((n-1) * k) + j ); // v[n-1][j]
      }
      addPrimitive(lineFanN,
                   lineFanS);
/*
