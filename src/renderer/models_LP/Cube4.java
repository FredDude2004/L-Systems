...
public class Cube4 extends Model
{
   ...
   */
   public Cube4(final int xCount, final int yGrid, final int zCount)
   {
      ...
      final Primitive bottomFan = new LineFan(),
                         topFan = new LineFan();
      ...
      for (int j = 0; j < 2*xCount + 2*zCount; ++j)
      {
         bottomFan.addIndex(j);                               // v[0][j]
            topFan.addIndex((yGrid+1)*(2*xCount+2*zCount)+j); // v[2+yGrid-1][j]
      }
      addPrimitive(bottomFan,
                      topFan);

      // Create all the line loops around the cube's vertical sides.
      for (int i = 0; i < 2 + yGrid; ++i) // choose a height of latitude
      {
         final Primitive lineLoop = new LineLoop();
         for (int j = 0; j < 2*xCount + 2*zCount; ++j)
         {
            lineLoop.addIndex(i*(2*xCount+2*zCount)+j); // v[i][j]
         }
         addPrimitive(lineLoop);
      }

      // Create all the vertical line strips in the cube's vertical sides.
      for (int j = 0; j < 2*xCount + 2*zCount; ++j)
      {
         final Primitive lineStrip = new LineStrip();
         for (int i = 0; i < 2 + yGrid; ++i) // choose a height of latitude
         {
            lineStrip.addIndex(i*(2*xCount+2*zCount)+j); // v[i][j]
         }
