...
/**
   Create a solid model of a partial right circular cone with its
...
<p>
   ...
   @see TorusSector
*/
public class ConeSector extends Model implements MeshMaker_TP
{
   ...
   */
   ...
   {
      this(r, h, h, theta1, theta2, n+1, k, MeshType.HORIZONTAL);
   }
   ...
   */
   public ConeSector(final double r,
                     ...
                     final int n, final int k)
   {
      this(r, h, t, theta1, theta2, n, k, MeshType.HORIZONTAL);
   }
   ...
   <p>
      ...
      counterclockwise direction).
   <p>
      The last two integer parameters determine the number of lines of longitude
      (not counting one edge of any removed sector) and the number of
      (partial) circles of latitude (not counting the top edge of the
      frustum) in the model.
   <p>
...
//    final int apexIndex = bottomCenterIndex + 1;
      ...
      // If t == h2, then this code will put degenerate triangles into
      // the triangle strip at the top (which should be a triangle fan).
...
//      for (int j = k - 1; j >= 0; --j)
//      {
//          topFan.addIndex( ((n-1)*k)+j );  // v[n-1][j]
//      }
//      topFan.addIndex( ((n-1)*k)+k-1 );  // v[n-1][k-1]
//      addPrimitive(topFan);
   }
   ...
   @Override
   public ConeSector remake(final int n, final int k, final MeshType type)
   {
