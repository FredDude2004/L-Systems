   ...
   */
   public Cube2(final int xGrid, final int yGrid, final int zGrid)
   {
      ...
      for (int i = 0; i < xGrid + 1; ++i)
      {
         ...
         {
            addPrimitive( new Triangles(
                                vT[i][j],     vT[i][j+1], vT[i+1][j+1],
                                vT[i+1][j+1], vT[i+1][j], vT[i][j]));

            addPrimitive( new Triangles(
                                vB[i][j],     vB[i+1][j], vB[i+1][j+1],
                                vB[i+1][j+1], vB[i][j+1], vB[i][j]));
         }
      }
      ...
      for (int i = 0; i < yGrid + 1; ++i)
      {
         ...
         {
            addPrimitive( new Triangles(
                                vF[i][j],     vF[i][j+1], vF[i+1][j+1],
                                vF[i+1][j+1], vF[i+1][j], vF[i][j]));
      ...
      for (int i = 0; i < yGrid + 1; ++i)
      {
         ...
         {
            addPrimitive( new Triangles(
                                vR[i][j],     vR[i+1][j], vR[i+1][j+1],
                                vR[i+1][j+1], vR[i][j+1], vR[i][j]));

            addPrimitive( new Triangles(
                                vL[i][j],     vL[i][j+1], vL[i+1][j+1],
                                vL[i+1][j+1], vL[i+1][j], vL[i][j]));
         }
      }
   }
}//Cube2
