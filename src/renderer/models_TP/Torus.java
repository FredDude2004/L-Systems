...
/**
   Create a solid model of a torus.
<p>
...
<p>
   ...
   @see TorusSector
*/
public class Torus extends Model implements MeshMaker_TP
{
   ...
   */
                ...
                final MeshType type)
   {
      ...
      if (MeshType.HORIZONTAL == type)
      {
         for (int i = 0; i < n; ++i)
         {
            ...
            for (int j = 0; j < k + 1; ++j)
            {
               triStrip.addIndex(   i *(k+1) + j);  // v[i  ][j]
               triStrip.addIndex((1+i)*(k+1) + j);  // v[i+1][j]
            }
            triStrip.addIndex(   i *(k+1) + 0);  // v[i  ][0]
            triStrip.addIndex((1+i)*(k+1) + 0);  // v[i+1][0]
      ...
      else if (MeshType.VERTICAL == type)
      {
         for (int j = 0; j < k; ++j)
         {
            ...
            for (int i = 0; i < n+1; ++i)
            {
               triStrip.addIndex(i * (k+1) + j+1);  // v[i][j+1]
               triStrip.addIndex(i * (k+1) + j);    // v[i][j  ]
            }
            addPrimitive(triStrip);
         }
         ...
         for (int i = 0; i < n+1; ++i)
         {
            triStrip.addIndex(i * (k+1) + k);  // v[i][k]
            triStrip.addIndex(i * (k+1) + 0);  // v[i][0]
         }
         addPrimitive(triStrip);
      }
      else // MeshType.CHECKER triangle strips
      {
         for (int i = 0; i < n; ++i)
         {
            ...
            {
               addPrimitive(
                  new TriangleStrip(   i *(k+1) + j,       // v[i  ][j  ]
                                    (1+i)*(k+1) + j,       // v[i+1][j  ]
                                       i *(k+1) + j + 1,   // v[i  ][j+1]
                                    (1+i)*(k+1) + j + 1)); // v[i+1][j+1]
            }
            addPrimitive(
               new TriangleStrip(   i *(k+1) + k,   // v[i  ][k]
                                 (1+i)*(k+1) + k,   // v[i+1][k]
                                    i *(k+1) + 0,   // v[i  ][0]
                                 (1+i)*(k+1) + 0)); // v[i+1][0]
         }
   ...
   @Override
   public Torus remake(final int n, final int k, final MeshType type)
   {
