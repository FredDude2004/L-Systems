...
public class PyramidFrustum extends Model implements MeshMaker
{
   ...
   */
   public PyramidFrustum(final double s1, final double s2, final double h)
   {
      ...
      // Create 12 triangles.
      addPrimitive(new Triangle(0, 1, 2), // 2 top triangles
                   new Triangle(2, 3, 0),
                   new Triangle(0, 4, 1), // 8 side triangles
                   new Triangle(4, 5, 1), // (2 per side)
                   new Triangle(1, 5, 2),
                   new Triangle(5, 6, 2),
                   new Triangle(2, 6, 3),
                   new Triangle(6, 7, 3),
                   new Triangle(3, 7, 0),
                   new Triangle(7, 4, 0),
                   new Triangle(4, 6, 5), // 2 base triangles
                   new Triangle(6, 4, 7));
   }
   ...
   */
                         ...
                         final int n, final int k)
   {
      ...
      for (int j = 0; j < 4*k; ++j)
      {  //                                          v[n-1][j+1]        v[n-1][j]
         addPrimitive(new Triangle(topCenterIndex, (n-1)*(4*k+1)+j+1, (n-1)*(4*k+1)+j));
      }
   }
