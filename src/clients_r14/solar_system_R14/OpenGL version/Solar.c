/*
   I changed some of the comments (but not any of the code)
   in the Animate() function to be clearer about the use of
   nested tranformations.
*/
 ...
 *    The up arrow key and down array key control the time
 *        step used in the animation rate.  Each key press
 *        multiplies or divides the times by a factor of two (2).
...
static void Animate(void)
{
   ...
   // 1) Position the Earth, moon system around the sun
   //    Use DayOfYear to determine its position
   glRotatef( 360.0*DayOfYear/365.0, 0.0, 1.0, 0.0 );
   glTranslatef( 4.0, 0.0, 0.0 );  // place origin at the center of the earth

     // 2) Draw the Earth.
     glPushMatrix();  // Save matrix state
     //  2a) Rotate the earth on its axis.
     //      Use HourOfDay to determine its rotation.
     glRotatef( 360.0*HourOfDay/24.0, 0.0, 1.0, 0.0 );  // spin the earth
     // 2b) Draw the earth as a wireframe sphere.
       glColor3f( 0.2, 0.2, 1.0 );
       glutWireSphere( 0.4, 10, 10 );
     glPopMatrix();  // Restore matrix state (origin at the center of the earth)

     // 3) Position the moon around the earth
     //    Use DayOfYear to control its rotation around the earth
     glRotatef( 360.0*12.0*DayOfYear/365.0, 0.0, 1.0, 0.0 );
     glTranslatef( 0.7, 0.0, 0.0 ); // place origin at the center of the moon
       // 3a) Draw the moon as a wireframe sphere.
