...
import renderer.scene.util.WireFrame;
...
*/
public abstract class InteractiveAbstractClient_R17 implements
                                                    KeyListener,
                                                    ComponentListener
{
   ...
   private Model savedModel = null; // used to hold a PointCloud or WireFrame model
   ...
   @Override public void keyTyped(KeyEvent e)
   {
      ...
      {
         final Position p = scene.getPosition(currentModel);
         if (p.getModel() instanceof MeshMaker_TP)
         {
            final MeshMaker_TP model = (MeshMaker_TP)p.getModel();
            final int n = model.getHorzCount();
            final int k = model.getVertCount();
            final MeshType type = model.getType().next();
            final Model newM = model.remake(n, k, type);
      ...
      else if ('P' == c)  // convert to point cloud
      {
         if (savedModel != null)
         {
      ...
      else if ('K' == c)  // convert to wireframe
      {
         if (savedModel != null)
         {
            scene.getPosition(currentModel).setModel(savedModel);
      ...
      else if ('c' == c && e.isAltDown())
      {
         // Change the solid random front face color of the current model.
         final Model m = scene.getPosition(currentModel).getModel();
         ModelShading.setRandomColorShaded(m);
         // don't need to reset backface colors
      }
      else if ('c' == c)
      {
         // Change the solid random front face color of the current model.
      ...
      {
         // Change each front face color in current model to a random color.
         final Model m = scene.getPosition(currentModel).getModel();
         ModelShading.setRandomColors(m);
         // don't need to reset backface colors
      }
      else if ('e' == c && e.isAltDown())
      {
         ...
         final Model m = scene.getPosition(currentModel).getModel();
         ModelShading.setRandomVertexColors(m);
         ModelShading.setBackFaceColor(m, backFaceColor,  // light green
                                          backFaceColor.brighter().brighter(),
                                          backFaceColor.darker().darker());
      }
      else if ('e' == c)
      {
         // Change the solid random color of each Primitive of the current model.
         final Model m = scene.getPosition(currentModel).getModel();
         ModelShading.setRandomPrimitiveColors(m);
         ModelShading.setBackFaceColor(m, backFaceColor,  // light green
                                          backFaceColor.brighter().brighter(),
                                          backFaceColor.darker().darker());
      }
      else if ('E' == c)
      {
         ...
         final Model m = scene.getPosition(currentModel).getModel();
         ModelShading.setRainbowPrimitiveColors(m);
         ModelShading.setBackFaceColor(m, backFaceColor,  // light green
                                          backFaceColor.brighter().brighter(),
                                          backFaceColor.darker().darker());
      }
             ...
             ||'.' == c || ',' == c)
      {
         ...
         if (p.getModel() instanceof MeshMaker)
         {
            ...
            try
            {
               ...
               if (p.getModel() instanceof MeshMaker_TP)
               {
                  final MeshType type = ((MeshMaker_TP)model).getType();
                  newM =((MeshMaker_TP)model).remake(n, k, type);
               }
               else
               {
                  newM = model.remake(n, k);
               }
