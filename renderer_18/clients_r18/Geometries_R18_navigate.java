...
*/
public class Geometries_R18_navigate implements
                                     KeyListener,
                                     ComponentListener,
                                     ActionListener
{
   ...
   */
   public Geometries_R18_navigate()
   {
    ...
    //final Texture texture = new Texture(assets + "textures/uv_grid_128.ppm");
      ...
      // row 2
      model[2][0] = new renderer.models_Tex.Torus(texture, 0.75, 0.25, 30, 30);
      ...
      model[2][2] = new renderer.models_Tex.Cube(texture);
      model[2][2] = model[2][2].transform(Matrix.scale(0.5, 0.5, 0.5)
                                   .times(Matrix.translate(1.0, 1.0, 1.0)));
      ...
      model[3][2].doBackFaceCulling = false;
      model[3][2].facesHaveTwoSides = false;

      // row 4 (last row in first image)
      model[4][0] = new renderer.models_Tex.Cone(texture, texture, 0.5, 1.0, 30, 30);
      ...
      model[4][2] = new renderer.models_Tex.Sphere(texture, 1.0, 30, 30);
      ...
      jf = new JFrame("Renderer 18 - Geometries navigate");
   ...
   private void setupViewing()
   {
      ...
      final Scene scene = new Scene("Geometries_R18_navigate_angle_"+angleNumber, camera);
      scene.debug = debug;
      ...
      if (0 == angleNumber) DrawSceneGraph.draw(scene, "SG_Geometries_R18_navigate");
