...
public class PanelYZ extends Model
{
   ...
   */
                  ...
                  final double x)
   {
      ...
      // Create the line strips that run in the z-direction.
      for (int y = 0; y <= yMax-yMin; ++y)
      {
         addPrimitive(new LineStrip(index[y]));
      }

      // Create the line strips that run in the y-direction.
      for (int z = 0; z <= zMax-zMin; ++z)
      {
         final int[] indices = new int[(yMax-yMin)+1];
         for (int y = 0; y <= yMax-yMin; ++y)
         {
            indices[y] = index[y][z];
         }
