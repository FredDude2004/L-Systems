...
/**
   Create a textured model of a parametric surface in space.
<p>
...
public class ParametricSurface extends Model implements MeshMaker
{
   ...
   /**
      Create a textured graph of the function with the following formula,
      <pre>{@code
   ...
   */
   public ParametricSurface(final Texture texture)
   {
      this(texture,
           (s,t) -> Math.sin(Math.PI*s) * Math.sin(Math.PI*t),
           -1.0, 1.0, -1.0, 1.0,
            49, 49);
   }
   ...
   /**
      Create a textured graph of a function of two variables
   ...
   */
   ...
   {
      this(texture, (x,z) -> x, f, (x,z) -> z, x1, x2, z1, z2, n, k);
   }


   /**
      Create a textured parametric surface in space,
      <pre>{@code
         x = x(s,t)
         y = y(s,t)
         z = z(s,t)
   ...
   */
                            ...
                            String name)
   {
      ...
      for (int i = 0; i < n; ++i) // choose a line of latitude
      {
         for (int j = 0; j < k; ++j) // choose a line of longitude
         {
            tc[i][j] = new TexCoord( i/(n - 1.0), j/(k - 1.0) );
         }
      }
      ...
      for (int i = 0; i < n - 1; ++i)
      {
         for (int j = 0; j < k - 1; ++j)
         {
            addPrimitive(new Triangle( (i * k) + j,   // v[i  ][j  ]
                                   ((i+1) * k) + j,   // v[i+1][j  ]
                                   ((i+1) * k) + j+1, // v[i+1][j+1]
                                       (i * k) + j,   // tc[i  ][j  ]
                                   ((i+1) * k) + j,   // tc[i+1][j  ]
                                   ((i+1) * k) + j+1, // tc[i+1][j+1]
                                   0));               // texture index
            addPrimitive(new Triangle( (i * k) + j,   // v[i  ][j  ]
                                   ((i+1) * k) + j+1, // v[i+1][j+1]
                                       (i * k) + j+1, // v[i  ][j+1]
   ...
   {
      return new ParametricSurface(this.texture,
