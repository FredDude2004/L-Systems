...
{
   private static final double Z_BUFFER_MAX_DEPTH = -Double.MAX_VALUE;
   ...
   public final double[] z_buffer;  // contains each pixel's depth data
   ...
   */
   public FrameBuffer(final int w, final int h, final Color c) {
      ...
      // Initialize the pixel and z buffer.
      this.bgColorFB = c;
      clearFB(c);
   ...
   */
   public FrameBuffer(final Viewport sourceVP) {
      ...
      // Read depth data, one pixel at a time, from the source Viewport.
      for (int y = 0; y < height; ++y) {
         for (int x = 0; x < width; ++x) {
            setDepthFB(x, y, sourceVP.getDepthVP(x,y));
         }
      }
   ...
   */
   ...
   /**
      Clear the {@code FrameBuffer}'s z-buffer (the depth buffer).
   */
   public void clearZB() {
      for (int y = 0; y < height; ++y) {
         for (int x = 0; x < width; ++x) {
            setDepthFB(x, y, Z_BUFFER_MAX_DEPTH);
         }
      }
   }
   ...
   */
   ...
   /**
      Get the depth from the z-buffer of the pixel with coordinates
      {@code (x,y)} in the {@code FrameBuffer}.

      @param x  horizontal coordinate within the {@code FrameBuffer}
      @param y  vertical coordinate within the {@code FrameBuffer}
      @return the depth of the current pixel at the given pixel coordinates
   */
   public double getDepthFB(final int x, final int y) {
      final int index = (y*width + x);
      try {
         return z_buffer[index];
      }
      catch(ArrayIndexOutOfBoundsException e) {
         System.err.println("Z-Buffer: Bad pixel coordinate (" + x + ", " + y +")");
       //e.printStackTrace(System.err);
         return Z_BUFFER_MAX_DEPTH;
      }
   }
   ...
   */
   ...
   /**
      Set the {@link Color} and depth of the pixel with coordinates
      {@code (x,y)} in the {@code FrameBuffer} and the z-buffer.

      @param x  horizontal coordinate within the {@code FrameBuffer}
      @param y  vertical coordinate within the {@code FrameBuffer}
      @param c  {@link Color} for the pixel at the given pixel coordinates
      @param z  depth for the pixel at the given pixel coordinates
   */
   public void setPixelFB(final int x, final int y, final Color c, final double z) {
      setPixelFB(x, y, c);
      setDepthFB(x, y, z);
   }
   ...
   */
   ...
   /**
      Set the combined RGB value and depth of the pixel with coordinates
      {@code (x,y)} in the {@code FrameBuffer}.
      ...
      @param z  depth for the pixel at the given pixel coordinates
   */
   public void setPixelFB(final int x, final int y, final int c, final double z) {
      setPixelFB(x, y, c);
      setDepthFB(x, y, z);
   }
   ...
   /**
      Set the depth in the z-buffer of the pixel with coordinates
      {@code (x,y)} in the {@code FrameBuffer}.
      ...
      @param z  depth for the pixel at the given pixel coordinates
   */
   public void setDepthFB(final int x, final int y, final double z) {
      final int index = (y*width + x);
      try {
         z_buffer[index] = z;
      }
   ...
   public class Viewport  // inner class (non-static nested class)
   {
      ...
      public Viewport(final int vp_ul_x, final int vp_ul_y,
         ...
         for (int y = 0; y < sourceFB.getHeightFB(); ++y) {
            for (int x = 0; x < sourceFB.getWidthFB(); ++x) {
               ...
               this.setDepthVP(x, y, sourceFB.getDepthFB(x,y));
            }
         }
      }
      ...
      public Viewport(final int vp_ul_x, final int vp_ul_y,
         ...
         for (int y = 0; y < sourceVP.getHeightVP(); ++y) {
            for (int x = 0; x < sourceVP.getWidthVP(); ++x) {
               ...
               this.setDepthVP(x, y, sourceVP.getDepthVP(x,y));
            }
         }
      }
      ...
      */
      public void clearZVP() {
         ...
         for (int y = 0; y < hVP; ++y) {
            for (int x = 0; x < wVP; ++x) {
               setDepthVP(x, y, Z_BUFFER_MAX_DEPTH);
            }
         }
      }
      ...
      */
      ...
      /**
         Get the depth from the z-buffer of the pixel with coordinates
         {@code (x,y)} relative to the current viewport.

         @param x  horizontal coordinate within the current viewport
         @param y  vertical coordinate within the current viewport
         @return depth of the current pixel at the given viewport coordinates
      */
      public double getDepthVP(final int x, final int y) {
         return getDepthFB(vp_ul_x + x, vp_ul_y + y);
      }
      ...
      */
      ...
      /**
         ...
         @param z  depth for the pixel at the given viewport coordinates
      */
      public void setPixelVP(final int x, final int y, final Color c, final double z)
      {
         setPixelVP(x, y, c);
         setDepthVP(x, y, z);
      }
      ...
      */
      ...
      /**
         ...
         @param c  combined RGB value for the pixel at the given {@code Viewport} coordinates
      */
      public void setPixelVP(final int x, final int y, final int c, final double z) {
         setPixelVP(x, y, c);
         setDepthVP(x, y, z);
      }
      ...
      /**
         ...
         @param z  depth for the pixel at the given viewport coordinates
      */
      public void setDepthVP(final int x, final int y, final double z) {
         setDepthFB(vp_ul_x + x, vp_ul_y + y, z);
      }
      ...
      public FrameBuffer convertVP2FB() {
         ...
         for (int y = 0; y < hVP; ++y) {
            for (int x = 0; x < wVP; ++x) {
               ...
               vp_fb.setDepthFB( x, y, this.getDepthVP(x, y) );
            }
         }
