   ...
   */
   public static boolean checkPrimitives(final Model model)
   {
      ...
      for (final Primitive p : model.primitiveList)
      {
         ...
         // Check back face colors.
         if (p instanceof Face)
         {
            final Face p2 = (Face)p;
            for (int i = 0; i < p2.cIndexList2.size(); ++i)
            {
               if (i >= numberOfColors)
               {
                  System.out.println("This Face Primitive has invalid back face color index: " + i);
         ...
         {
            final Triangle p2 = (Triangle)p;
            for (int i = 0; i < p2.cIndexList2.size(); ++i)
            {
               if (i >= numberOfColors)
               {
                  System.out.println("This Triangle Primitive has invalid back face color index: " + i);
                  System.out.println( p2 );
                  result = false;
               }
            }
