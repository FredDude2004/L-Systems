...
/**
   Create a solid model of a right equilateral triangular prism
...
public class TriangularPrism extends Model
{
   ...
   */
   public TriangularPrism(final double r,
                          final double h, final double theta,
                          final int n)
   {
   ...
   */
                          ...
                          final boolean bothHalves)
   {
      ...
      // Create 6+6(n+1) = 6(n+2) triangles,
      // 2 triangles for each of the 3(n+1) faces of the prism,
      // 3 triangles at the top,
      // 3 triangles at the bottom.

      // 3 top faces
      addPrimitive(new Triangle(top, 3*(n+1)+1, 3*(n+1)+0),
                   new Triangle(top, 3*(n+1)+2, 3*(n+1)+1),
                   new Triangle(top, 3*(n+1)+0, 3*(n+1)+2));
      // 3 bottom faces
      addPrimitive(new Triangle(bot, 0, 1),
                   new Triangle(bot, 1, 2),
                   new Triangle(bot, 2, 0));

      // Create all the triangle strips along the prism wall.
      for (int i = 0; i < n+1; ++i)
      {
         addPrimitive(new Triangle(3*i + 0,     3*(i+1) + 0, 3*(i+1) + 1),
                      new Triangle(3*(i+1) + 1, 3*i + 1,     3*i + 0));

         addPrimitive(new Triangle(3*i + 1,     3*(i+1) + 1, 3*(i+1) + 2),
                      new Triangle(3*(i+1) + 2, 3*i + 2,     3*i + 1));

         addPrimitive(new Triangle(3*i + 2,     3*(i+1) + 2, 3*(i+1) + 0),
                      new Triangle(3*(i+1) + 0, 3*i + 0,     3*i + 2));
      }
   }
}//TriangularPrism
