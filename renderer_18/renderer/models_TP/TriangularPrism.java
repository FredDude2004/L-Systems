...
public class TriangularPrism extends Model
{
   ...
   */
                          ...
                          final boolean bothHalves)
   {
      ...
      // 3 top faces
      addPrimitive(new TriangleFan(top, 3*(n+1)+1, 3*(n+1)+0, 3*(n+1)+2, 3*(n+1)+1));
      // 3 bottom faces
      addPrimitive(new TriangleFan(bot, 0, 1, 2, 0));

      // Create all the triangle strips along the prism wall.
      for (int i = 0; i < n+1; ++i)
      {
         // these 6 triangles should be one triangle strip
