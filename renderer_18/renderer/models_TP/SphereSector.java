...
/**
   Create a solid model of a partial sphere centered at the origin
<p>
...
<p>
   ...
   @see TorusSector
*/
public class SphereSector extends Model implements MeshMaker_TP
{
   ...
   */
   public SphereSector(final double r,
                       ...
                       final int n, final int k)
   {
      this(r, theta1, theta2, phi1, phi2, n, k, MeshType.HORIZONTAL);
   }
   ...
   */
                       ...
                       final MeshType type)
   {
      ...
      // Create all the triangle strips between phi1 and phi2.
      // If phi1 == 0 or phi2 == Math.PI, then this code will
      // put degenerate triangles into the triangle strip at
      // the north or south pole (which should be a triangle fan).
      if (MeshType.HORIZONTAL == type)
      {
         for (int i = 0; i < n - 1; ++i) // choose a circle of latitude
         {
            final Primitive triStrip = new TriangleStrip();
            for (int j = 0; j < k; ++j) // choose a half-circle of longitude
            {
      ...
      {
         for (int j = 0; j < k - 1; ++j) // choose a half-circle of longitude
         {
            final Primitive triStrip = new TriangleStrip();
...
/*
      // Create all the triangle strips between phi1 and phi2
      // while preventing degenerate triangles.
      ...
      for (int i = 0; i < n - 1; ++i)
      {
         ...
         for (int j = 0; j < k - 1; ++j)
         {
            if ( (i != 0) || (phi1 != 0) ) // prevent degenerate triangles
            {  //          v[i][j]   v[i][j+1]  v[i+1][j+1]
               addTriangle((i*k)+j, (i*k)+j+1, ((i+1)*k)+j+1);
            }
            if ( (i != n-2) || (phi2 != Math.PI) ) // prevent degenerate triangles
            {  //           v[i+1][j+1]    v[i+1][j]   v[i][j]
               addTriangle(((i+1)*k)+j+1, ((i+1)*k)+j, (i*k)+j);
            }
         }
         addPrimitive(triStrip);
      }
      if (phi1 == 0)       addPrimitive(topFan);
      if (phi2 == Math.PI) addPrimitive(botFan);
*/
   }
   ...
   @Override
   public SphereSector remake(final int n, final int k, final MeshType type)
   {
