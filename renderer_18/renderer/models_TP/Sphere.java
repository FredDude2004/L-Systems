...
<p>
   ...
   @see SphereSector
*/
public class Sphere extends Model implements MeshMaker_TP
{
   ...
   */
                 ...
                 final MeshType type)
   {
      ...
      // Create all the triangle strips between the top and bottom poles.
      else if (MeshType.VERTICAL == type)
      {
         for (int j = 0; j < k - 1; ++j) // choose a half-circle of longitude
         {
            final Primitive triStrip = new TriangleStrip();
            triStrip.addIndex(northPoleIndex);
            for (int i = 0; i < n - 1; ++i) // choose a circle of latitude
            {
               triStrip.addIndex(i * k + j);    // v[i][j  ]
               triStrip.addIndex(i * k + j+1);  // v[i][j+1]
            }
            triStrip.addIndex(southPoleIndex);
            addPrimitive(triStrip);
         }
         // Last vertical strip to close the sphere.
         final Primitive triStrip = new TriangleStrip();
         triStrip.addIndex(northPoleIndex);
         for (int i = 0; i < n - 1; ++i) // choose a circle of latitude
         {
   ...
   @Override
   public Sphere remake(final int n, final int k, final MeshType type)
   {
      return new Sphere(this.r, n, k, type);
   }
}//Sphere
